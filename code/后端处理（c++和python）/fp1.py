...
if __name__ == '__main__':
    ...
    with open("edge-book.json", "r", encoding='utf-8') as file:
        data = json.load(file)
        print("ok")

    # print(data[0])
    fp = FP(minsup=0) # 这里是设置占总数的百分比，忘记调了，跑的时候可能会有问题,设置为0就是全部都要建树
    fp.get_fre_set(data)

    # 结果输出
    result=[]
    with open("result-book.json", "w", encoding='utf-8') as file:
        for itemset, sup in zip(fp.fre_itemsets, fp.fre_itemsets_sups):
            # print(sup);
            if len(itemset) > 2:
                # result.append(len(itemset))
                # print(len(itemset))
                # print(itemset)
