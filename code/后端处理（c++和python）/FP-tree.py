...
with open("111.json", "r", encoding='utf-8') as file:
    data = json.load(file)
...
support = 3
# 统计item的频率
CountItem = collections.defaultdict(int) # 就是一个dict
for line in data:
    for item in line:
        # print(item)
        CountItem[item] += 1
...
a = sorted(CountItem.items(), key=lambda x: x[1], reverse=True) # a[i]:('f', 4) 应该是存了一个元组
